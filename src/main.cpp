...
typedef struct _OBJECT_DIRECTORY_INFORMATION { UNICODE_STRING Name; UNICODE_STRING TypeName; } OBJECT_DIRECTORY_INFORMATION, *POBJECT_DIRECTORY_INFORMATION;
...
struct IfHit { bool ok; USHORT vMaj; USHORT vMin; };
struct EndpointInfo { wstring scope; DWORD sessionId; wstring name; wstring ntPath; };

static void initUnicode(UNICODE_STRING& us, const wchar_t* s) {
    if (!s) { us.Buffer = nullptr; us.Length = us.MaximumLength = 0; return; }
...
static void enumObjectDirectory(const wstring& ntPath, vector<wstring>& outNames) {
    ...
    for (;;) {
        ULONG retLen = 0;
        NTSTATUS st = NtQueryDirectoryObject(h, buf.data(), bufSize, TRUE, restart, &ctx, &retLen);
        restart = FALSE;
...
static IfHit probeEndpoint(const wstring& endpointName, const GUID& target, unsigned long timeoutMs) {
    ...
    RPC_STATUS rs = RpcBindingFromStringBindingW(s, &h);
    if (s) RpcStringFreeW(&s);
    if (rs != RPC_S_OK) return {false, 0, 0};

    RpcBindingSetOption(h, RPC_C_OPT_CALL_TIMEOUT, timeoutMs);

    RPC_IF_ID_VECTOR* ifs = nullptr;
    rs = RpcMgmtInqIfIds(h, &ifs);
    if (rs != RPC_S_OK || !ifs) { RpcBindingFree(&h); return {false, 0, 0}; }

    IfHit hit{false, 0, 0};
    for (unsigned long i = 0; i < ifs->Count; i++) {
        RPC_IF_ID* iid = ifs->IfId[i];
        if (!iid) continue;
        if (InlineIsEqualGUID(iid->Uuid, target)) { hit = {true, iid->VersMajor, iid->VersMinor}; break; }
    }
...
static void collectEndpoints(bool useSpecificSession,
                             DWORD specificSession,
                             DWORD currentSession,
                             vector<EndpointInfo>& out) {
...
int wmain(int argc, wchar_t** argv) {
    ...
    for (int i = 2; i < argc; i++) {
        if (_wcsicmp(argv[i], L"--session") == 0 && i + 1 < argc) { specificSession = static_cast<DWORD>(_wtoi(argv[++i])); useSpecificSession = true; }
        else if (_wcsicmp(argv[i], L"--timeout") == 0 && i + 1 < argc) { callTimeoutMs = static_cast<unsigned long>(_wtol(argv[++i])); }
        else if (_wcsicmp(argv[i], L"--endpoint") == 0 && i + 1 < argc) { onlyEndpoint = argv[++i]; }
        else if (_wcsicmp(argv[i], L"--no-skip") == 0) { noSkip = true; }
    }
    ...
    const unordered_set<wstring> defaultSkip = {
        L"LSARPC_ENDPOINT", L"lsapolicylookup", L"samss lpc", L"securityevent",
        L"audit", L"lsasspirpc", L"LSA_IDPEXT_ENDPOINT", L"LSA_EAS_ENDPOINT",
        L"protected_storage"
    };
    ...
    wcout << L"Scanning " << endpoints.size() << L" endpoints for " << targetStr
          << L"  (timeout=" << callTimeoutMs << L"ms)\n";

    wcout << left
          << setw(static_cast<int>(scopeWidth)) << L"Scope" << L"  "
          << setw(static_cast<int>(sessWidth))  << L"Session" << L"  "
          << setw(static_cast<int>(epWidth))    << L"Endpoint" << L"  "
          << setw(static_cast<int>(verWidth))   << L"IfVersion" << L"  "
    ...
    for (const auto& ep : endpoints) {
        if (!noSkip && onlyEndpoint.empty() && defaultSkip.find(ep.name) != defaultSkip.end()) { skipped++; continue; }
        scanned++;

        IfHit hit = probeEndpoint(ep.name, target, callTimeoutMs);
        if (hit.ok) {
            wcout << left
                  << setw(static_cast<int>(scopeWidth)) << ep.scope << L"  ";
            if (ep.scope == L"Session") {
                wcout << setw(static_cast<int>(sessWidth)) << ep.sessionId << L"  ";
            } else {
                wcout << setw(static_cast<int>(sessWidth)) << L"-" << L"  ";
            }
            wcout << setw(static_cast<int>(epWidth))  << ep.name << L"  "
                  << setw(static_cast<int>(verWidth)) << (to_wstring(hit.vMaj) + L"." + to_wstring(hit.vMin)) << L"  "
                  << setw(static_cast<int>(protoWidth)) << L"ncalrpc" << L"\n";
            hits++;
...
void* __RPC_USER midl_user_allocate(size_t s) { return HeapAlloc(GetProcessHeap(), 0, s); }
void  __RPC_USER midl_user_free(void* p) { if (p) HeapFree(GetProcessHeap(), 0, p); }
