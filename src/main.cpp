...
extern "C"
{
	...
	__declspec(dllimport) NTSTATUS __stdcall NtQueryDirectoryObject(_In_ HANDLE, _Out_writes_bytes_opt_(Length) PVOID, _In_ ULONG, _In_ BOOLEAN,
																	_In_ BOOLEAN, _Inout_ PULONG, _Out_opt_ PULONG);
}
...
static void enumObjectDirectory(const wstring& ntPath, vector<wstring>& outNames) {
	...
	HANDLE h = nullptr;
	if (!NT_SUCCESS(NtOpenDirectoryObject(&h, DIRECTORY_QUERY, &oa)))
		return;
...
static IfHit probeEndpoint(const wstring& endpointName, const GUID& target, unsigned long timeoutMs) {
	RPC_WSTR s = nullptr;
	if (RpcStringBindingComposeW(nullptr, reinterpret_cast<RPC_WSTR>(const_cast<wchar_t*>(L"ncalrpc")), nullptr,
								 reinterpret_cast<RPC_WSTR>(const_cast<wchar_t*>(endpointName.c_str())), nullptr, &s) != RPC_S_OK) {
		return {false, 0, 0};
	}
...
static void collectEndpoints(bool useSpecificSession, DWORD specificSession, DWORD currentSession, vector<EndpointInfo>& out) {
	...
	for (auto& n : names)
		out.push_back(EndpointInfo{L"Global", DWORD(-1), n, L"\\RPC Control"});
	...
	auto scanSession = [&](DWORD sid) {
		...
		for (auto& n : local)
			out.push_back(EndpointInfo{L"Session", sid, n, path});
	};
...
int wmain(int argc, wchar_t** argv) {
	...
	const unordered_set<wstring> defaultSkip = {L"LSARPC_ENDPOINT", L"lsapolicylookup",		L"samss lpc",		 L"securityevent",	  L"audit",
												L"lsasspirpc",		L"LSA_IDPEXT_ENDPOINT", L"LSA_EAS_ENDPOINT", L"protected_storage"};

	size_t epWidth = 8, scopeWidth = 6, verWidth = 7, protoWidth = 8, sessWidth = 8;
	for (const auto& e : endpoints)
		epWidth = max(epWidth, e.name.size());
	...
	for (const auto& ep : endpoints) {
		...
		if (hit.ok) {
			...
			wcout << setw(static_cast<int>(epWidth)) << ep.name << L"  " << setw(static_cast<int>(verWidth))
				  << (to_wstring(hit.vMaj) + L"." + to_wstring(hit.vMin)) << L"  " << setw(static_cast<int>(protoWidth)) << L"ncalrpc" << L"\n";
			hits++;
			if (!onlyEndpoint.empty())
				break;
		}
